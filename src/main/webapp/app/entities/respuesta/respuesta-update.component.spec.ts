/* tslint:disable max-line-length */
import { vitest } from 'vitest';
import { type MountingOptions, shallowMount } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';
import { type RouteLocation } from 'vue-router';

import dayjs from 'dayjs';
import RespuestaUpdate from './respuesta-update.vue';
import RespuestaService from './respuesta.service';
import { DATE_TIME_LONG_FORMAT } from '@/shared/composables/date-format';
import AlertService from '@/shared/alert/alert.service';

import PqrsService from '@/entities/pqrs/pqrs.service';

type RespuestaUpdateComponentType = InstanceType<typeof RespuestaUpdate>;

let route: Partial<RouteLocation>;
const routerGoMock = vitest.fn();

vitest.mock('vue-router', () => ({
  useRoute: () => route,
  useRouter: () => ({ go: routerGoMock }),
}));

const respuestaSample = { id: 'ABC' };

describe('Component Tests', () => {
  let mountOptions: MountingOptions<RespuestaUpdateComponentType>['global'];
  let alertService: AlertService;

  describe('Respuesta Management Update Component', () => {
    let comp: RespuestaUpdateComponentType;
    let respuestaServiceStub: SinonStubbedInstance<RespuestaService>;

    beforeEach(() => {
      route = {};
      respuestaServiceStub = sinon.createStubInstance<RespuestaService>(RespuestaService);
      respuestaServiceStub.retrieve.onFirstCall().resolves(Promise.resolve([]));

      alertService = new AlertService({
        i18n: { t: vitest.fn() } as any,
        bvToast: {
          toast: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          'font-awesome-icon': true,
          'b-input-group': true,
          'b-input-group-prepend': true,
          'b-form-datepicker': true,
          'b-form-input': true,
        },
        provide: {
          alertService,
          respuestaService: () => respuestaServiceStub,
          pqrsService: () =>
            sinon.createStubInstance<PqrsService>(PqrsService, {
              retrieve: sinon.stub().resolves({}),
            } as any),
        },
      };
    });

    afterEach(() => {
      vitest.resetAllMocks();
    });

    describe('load', () => {
      beforeEach(() => {
        const wrapper = shallowMount(RespuestaUpdate, { global: mountOptions });
        comp = wrapper.vm;
      });
      it('Should convert date from string', () => {
        // GIVEN
        const date = new Date('2019-10-15T11:42:02Z');

        // WHEN
        const convertedDate = comp.convertDateTimeFromServer(date);

        // THEN
        expect(convertedDate).toEqual(dayjs(date).format(DATE_TIME_LONG_FORMAT));
      });

      it('Should not convert date if date is not present', () => {
        expect(comp.convertDateTimeFromServer(null)).toBeNull();
      });
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', async () => {
        // GIVEN
        const wrapper = shallowMount(RespuestaUpdate, { global: mountOptions });
        comp = wrapper.vm;
        comp.respuesta = respuestaSample;
        respuestaServiceStub.update.resolves(respuestaSample);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(respuestaServiceStub.update.calledWith(respuestaSample)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', async () => {
        // GIVEN
        const entity = {};
        respuestaServiceStub.create.resolves(entity);
        const wrapper = shallowMount(RespuestaUpdate, { global: mountOptions });
        comp = wrapper.vm;
        comp.respuesta = entity;

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(respuestaServiceStub.create.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        respuestaServiceStub.find.resolves(respuestaSample);
        respuestaServiceStub.retrieve.resolves([respuestaSample]);

        // WHEN
        route = {
          params: {
            respuestaId: `${respuestaSample.id}`,
          },
        };
        const wrapper = shallowMount(RespuestaUpdate, { global: mountOptions });
        comp = wrapper.vm;
        await comp.$nextTick();

        // THEN
        expect(comp.respuesta).toMatchObject(respuestaSample);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        respuestaServiceStub.find.resolves(respuestaSample);
        const wrapper = shallowMount(RespuestaUpdate, { global: mountOptions });
        comp = wrapper.vm;
        await comp.$nextTick();

        comp.previousState();
        await comp.$nextTick();

        expect(routerGoMock).toHaveBeenCalledWith(-1);
      });
    });
  });
});
